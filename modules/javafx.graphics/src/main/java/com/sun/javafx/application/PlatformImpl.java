...
//                System.err.println("PlatformImpl.tkExit: calling Toolkit.exit");
    ...
    private static List<Stylesheet> themeStylesheets;

    private static final ListChangeListener<Stylesheet> themeStylesheetsChanged = change -> {
     ...
     */
    private static void updateStyleManager(String userAgentStylesheet, List<Stylesheet> stylesheets) {
        if (themeStylesheets != stylesheets) {
            if (themeStylesheets instanceof ObservableList<Stylesheet> list) {
        ...
        if (hasUserAgentStylesheet) {
            try {
                Stylesheet stylesheet = Stylesheet.load(URI.create(userAgentStylesheet));
                StyleManager.getInstance().setUserAgentStylesheets(List.of(stylesheet));
