 ...
 * @since JavaFX 8.0
 */
public abstract class StyleConverter<T> {

    private final Class<T> convertedType;
    private StyleConverter<T[]> sequenceConverter;

    protected StyleConverter(Class<T> convertedType) {
        this.convertedType = convertedType;
    }

    public abstract T convert(Block value, Font font);

    public final Class<T> convertedType() {
        return convertedType;
    }

    @SuppressWarnings("unchecked")
    public StyleConverter<T[]> getSequenceConverter() {
        if (sequenceConverter == null) {
            StyleConverter<T> parent = this;

            sequenceConverter = new StyleConverter<>((Class<T[]>)convertedType.arrayType()) {
                final T[] EMPTY = (T[])Array.newInstance(convertedType, 0);

                @Override
                public T[] convert(Block block, Font font) {
                    var values = new ArrayList<T>();

                    for (int i = 0, start = 0, max = block.size(); i < max; ++i) {
                        var value = block.get(i);

                        if (value instanceof CommaToken) {
                            if (i - start == 1) {
                                values.add(parent.convert(CssDeclaredValue.ofResolved(block.get(i)), font));
                            } else {
                                values.add(parent.convert(CssDeclaredValue.ofResolved(block.subList(start, i)), font));
                            }
