...
public class ImageUtilsTest {
    ...
    void fullyTransparentReturnsBackground() {
        ...
        assertClose(result, 255, 255, 255);
    }
    ...
    void majorityColorWins() {
        ...
        assertClose(result, 0, 255, 0); // should be green
    }
    ...
    void transparentAreaCanMakeBackgroundDominant() {
        ...
        var result = ImageUtils.computeDominantColor(img, Color.WHITE, 2, 1, 0);
        assertClose(result, 255, 255, 255); // background dominates
    }

    @Test
    void partialAlphaCompositesOverBackground() {
        // 50% alpha red (A=0x80) over white is composited in linear light
        var img = new WritableImage(16, 16);
        fillArgb(img, 0x80ff0000);
        var result = ImageUtils.computeDominantColor(img, Color.WHITE, 2, 1, 0);
        assertClose(result, 255, 187, 187);
    }

    @Test
    void halfTransparentBlackOverWhite_usesLinearLightBlending() {
        ...
        // With linear-light blending, 50% black over white converts back to 187 in sRGB.
        assertClose(result, 187, 187, 187);
    }
    ...
    private static void assertClose(Color actual, int r, int g, int b) {
        ...
        assertTrue(Math.abs(r8 - r) <= 1, "R expected " + r + ", got " + r8);
        assertTrue(Math.abs(g8 - g) <= 1, "G expected " + g + ", got " + g8);
        assertTrue(Math.abs(b8 - b) <= 1, "B expected " + b + ", got " + b8);
    }
}
