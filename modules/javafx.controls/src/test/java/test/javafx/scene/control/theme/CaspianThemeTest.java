...
public class CaspianThemeTest {
    ...
    public void testHighContrastThemeWithSystemProperty() {
        ...
        assertFalse(theme.getStylesheets().stream().anyMatch(s -> s.getUrl().contains("highcontrast")));
        System.setProperty("com.sun.javafx.highContrastTheme", "ANY_VALUE_HERE");
        theme = new CaspianTheme();
        assertTrue(theme.getStylesheets().stream().anyMatch(s -> s.getUrl().contains("highcontrast")));
    ...
    public void testHighContrastThemeWithPlatformPreference() {
        ...
        prefs.update(Map.of(
            ...
            "Windows.SPI.HighContrastColorScheme", "ANY_VALUE_HERE"));
