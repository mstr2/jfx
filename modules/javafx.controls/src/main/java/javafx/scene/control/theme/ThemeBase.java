 ...
 * @since 21
 */
public abstract class ThemeBase extends StyleTheme {

    private static final String SKIN_PREFIX = "com/sun/javafx/scene/control/skin/";
    ...
    @Override
    public final ObservableList<Stylesheet> getStylesheets() {
     ...
     */
    protected final WritableValue<Stylesheet> addFirst(Stylesheet url) {
     ...
     * @return a {@code WritableValue} that represents the stylesheet URL in the list of stylesheets
     */
    protected final WritableValue<Stylesheet> addLast(Stylesheet url) {
        return stylesheetList.addLastElement(url);
    }

    protected final Stylesheet loadThemeStylesheet(String name) throws IOException {
        return Stylesheet.load(URI.create(name), this::getResource);
    }

    @Override
    public URL getResource(String name) {
        return Utils.getResource(name.substring(SKIN_PREFIX.length()));
    }
